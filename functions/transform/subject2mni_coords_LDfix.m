...
if ispc
    ...
    if exist(activate_bat, 'file')
        cmd = sprintf('call "%s" %s && subject2mni_coords -m "%s" -s "%s" -o "%s" -t %s', ...
            activate_bat, env_name, subdir, fn_in, fn_out, transformation_type);
    else
        cmd = sprintf('"%s%ssubject2mni_coords" -m "%s" -s "%s" -o "%s" -t %s', ...
            parameters.simnibs_bin_path, filesep, subdir, fn_in, fn_out, transformation_type);
    end
else
    cmd = sprintf('%s%s/subject2mni_coords -m %s -s %s -o %s -t %s;', ...
